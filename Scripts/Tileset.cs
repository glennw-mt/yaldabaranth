...
public class Tileset
{
  ...
  public Tileset(string path)
  {
    ...
    for (int x = 0; x < tileset_width; x++) for (int y = 0; y < tileset_height; y++)
    {
      Color pixel = image.GetPixel(x, y);
      if (!(pixel.A == 0 || pixel == Color.Color8(0, 0, 0))) image.SetPixel(x, y, Color.Color8(255, 255, 255));
      if (pixel == Color.Color8(0, 0, 0)) image.SetPixel(x, y, Color.Color8(0, 0, 0, 0));
    }
  }
  public ImageTexture GetTile(Tile tile)
  {
    ...
    switch (tile)
    {
      case Tile.Man: x = 104; y = 0; break;
      case Tile.Tree: x = 1; y = 8; break;
      default: x = 0; y = 0; break;
    }
    ImageTexture tile_texture;
    if (cache.ContainsKey(tile)) tile_texture = cache[tile];
    else
    {
